...
# Test dimensions
BATCH_HEADS = 2
Q_SEQ = 1024  # 8 Q groups of 128
K_SEQ = 1024  # fixed


def _cross_attn_body(q_ref, k_ref, v_ref, mask_ref):
    """Shared NKI cross-attention body used by both ref and test."""
    ...
    for bh in nl.affine_range(batch_heads):
        ...
        for qg in nl.sequential_range(n_q_grps):
            ...
            nisa.activation_reduce(
                e0, op=nl.exp, data=s0, bias=neg_max, reduce_op=nl.add, reduce_res=sum0
            )
            ...
            nisa.activation_reduce(
                e1, op=nl.exp, data=s1, bias=neg_max, reduce_op=nl.add, reduce_res=sumt
            )
            ...
            nisa.activation_reduce(
                e2, op=nl.exp, data=s2, bias=neg_max, reduce_op=nl.add, reduce_res=sumt
            )
            nisa.tensor_tensor(sum0, sum0, sumt, op=nl.add)
            ...
            nisa.activation_reduce(
                e3, op=nl.exp, data=s3, bias=neg_max, reduce_op=nl.add, reduce_res=sumt
            )
            nisa.tensor_tensor(sum0, sum0, sumt, op=nl.add)
            ...
            nisa.activation_reduce(
                e4, op=nl.exp, data=s4, bias=neg_max, reduce_op=nl.add, reduce_res=sumt
            )
            nisa.tensor_tensor(sum0, sum0, sumt, op=nl.add)
            ...
            nisa.activation_reduce(
                e5, op=nl.exp, data=s5, bias=neg_max, reduce_op=nl.add, reduce_res=sumt
            )
            nisa.tensor_tensor(sum0, sum0, sumt, op=nl.add)
            ...
            nisa.activation_reduce(
                e6, op=nl.exp, data=s6, bias=neg_max, reduce_op=nl.add, reduce_res=sumt
            )
            nisa.tensor_tensor(sum0, sum0, sumt, op=nl.add)
            ...
            nisa.activation_reduce(
                e7, op=nl.exp, data=s7, bias=neg_max, reduce_op=nl.add, reduce_res=sumt
            )
            nisa.tensor_tensor(sum0, sum0, sumt, op=nl.add)
...
@nki.jit
def ref(q_ref, k_ref, v_ref, mask_ref):
    """Reference NKI masked cross-attention."""
    return _cross_attn_body(q_ref, k_ref, v_ref, mask_ref)
...
@nki.jit
def test(q_ref, k_ref, v_ref, mask_ref):
    """Test NKI masked cross-attention (copy of ref, to be optimized)."""
    return _cross_attn_body(q_ref, k_ref, v_ref, mask_ref)


def cpu_reference(q_ref, k_ref, v_ref, mask_ref):
    """CPU reference: masked cross-attention."""
    batch_heads = q_ref.shape[0]
    q_seq = q_ref.shape[1]
    d = q_ref.shape[2]
    k_seq = k_ref.shape[1]

    out = np.zeros((batch_heads, q_seq, d), dtype=np.float32)
    q_np = q_ref.astype(np.float32)
    k_np = k_ref.astype(np.float32)
    v_np = v_ref.astype(np.float32)
    mask_np = mask_ref.astype(np.float32)

    for bh in range(batch_heads):
        # Q @ K^T: [q_seq, d] @ [d, k_seq] -> [q_seq, k_seq]
        scores = q_np[bh] @ k_np[bh].T  # [q_seq, 1024]
        # Add mask: broadcast [1024] -> [q_seq, 1024]
        scores += mask_np[bh][np.newaxis, :]
        # Softmax
        m = np.max(scores, axis=-1, keepdims=True)
        exp_s = np.exp(scores - m)
        s = exp_s / np.sum(exp_s, axis=-1, keepdims=True)
        # @ V
        out[bh] = s @ v_np[bh]
...
def test_nki(ref_func, test_func):
    ...
    for seed in [42, 123]:
        ...
        # Generate inputs: Q is pre-scaled
        scale = 1.0 / math.sqrt(_XATTN_D)
        q_np = (
            np.random.randn(BATCH_HEADS, Q_SEQ, _XATTN_D).astype(np.float32)
            * 0.1
            * scale
        )
        k_np = np.random.randn(BATCH_HEADS, K_SEQ, _XATTN_D).astype(np.float32) * 0.1
        v_np = np.random.randn(BATCH_HEADS, K_SEQ, _XATTN_D).astype(np.float32) * 0.1
        # Mask: 80% unmasked (0.0), 20% masked (-10000.0)
        mask_np = np.where(
            np.random.rand(BATCH_HEADS, K_SEQ) > 0.2, 0.0, -10000.0
        ).astype(np.float32)

        # CPU reference
        cpu_out = cpu_reference(q_np, k_np, v_np, mask_np)

        q_t = torch.tensor(q_np, dtype=torch.bfloat16).to(device)
        k_t = torch.tensor(k_np, dtype=torch.bfloat16).to(device)
        v_t = torch.tensor(v_np, dtype=torch.bfloat16).to(device)
...
def benchmark_nki(nki_func):
    ...
    scale = 1.0 / math.sqrt(_XATTN_D)
    q_np = (
        np.random.randn(BATCH_HEADS, Q_SEQ, _XATTN_D).astype(np.float32) * 0.1 * scale
    )
    k_np = np.random.randn(BATCH_HEADS, K_SEQ, _XATTN_D).astype(np.float32) * 0.1
    v_np = np.random.randn(BATCH_HEADS, K_SEQ, _XATTN_D).astype(np.float32) * 0.1
    mask_np = np.where(np.random.rand(BATCH_HEADS, K_SEQ) > 0.2, 0.0, -10000.0).astype(
        np.float32
    )
