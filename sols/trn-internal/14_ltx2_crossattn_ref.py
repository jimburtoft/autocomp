...
K_seq = 1024, d = 128, Q_seq variable (6144 for Stage 1, 24576 for Stage 2).
Mask: [batch_heads, 1024] additive bias {0, -10000} (2D, flat).

Uses ONLY nisa.* (ISA-level) APIs. All loads/stores via nisa.dma_copy,
transposes via nisa.nc_transpose, math via nisa.tensor_tensor /
nisa.tensor_scalar / nisa.tensor_reduce / nisa.activation_reduce / nisa.nc_matmul.
"""
...
def cross_attn_kernel(q_ref, k_ref, v_ref, mask_ref):
    ...
    for bh in nl.affine_range(batch_heads):
        # Load & transpose K: 8 x [128,128] -> [d_par, 128]
        _kc0_bf = nl.ndarray(
        ...
        # Load V: 8 x [128_par, 128_free] in float32
        _vt0_bf = nl.ndarray(
        ...
        # Load mask chunks and broadcast via outer product
